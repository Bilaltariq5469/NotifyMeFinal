...
//    static Double latitude=31.56781;
    ...
    private NotificationManager roadConditionNotificationManager;
    ...
    {
        //Toast.makeText(context, "Checking Cordinates for Road Point", Toast.LENGTH_SHORT).show();
        ArrayList<RoadCondition> roadconditionlist = roadConditionDb.getRoadList();
        for(int i=0;i<roadconditionlist.size();i++) {
            Location savedlocation = new Location("");
            savedlocation.setLongitude(Double.parseDouble(roadconditionlist.get(i).getCordinates().split("/")[1]));
            savedlocation.setLatitude(Double.parseDouble(roadconditionlist.get(i).getCordinates().split("/")[0]));
            if ((currentlocation.distanceTo(savedlocation) / 1000) <= 1) {
                // change its color from red to green
                sendNotification(roadconditionlist.get(i).getMessage());
            }
        }
        //context.stopService(new Intent(context,RoadConditionService.class));
    }
    @Override
    ...
    private void sendNotification(String msg) {
        Log.d("RoadConditionService", "Preparing to send notification...: " + msg);
        roadConditionNotificationManager = (NotificationManager) this
...
//                new Intent(this, AddAlarmActivity.class), 0);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            roadConditionNotificationManager.createNotificationChannel(channel);
        }
        NotificationCompat.Builder alamNotificationBuilder = new NotificationCompat.Builder(
                this).setContentTitle("Road Status").setSmallIcon(android.R.drawable.arrow_down_float)
                .setStyle(new NotificationCompat.BigTextStyle().bigText(msg))
                .setContentText(msg);


        alamNotificationBuilder.setContentIntent(null);
        roadConditionNotificationManager.notify(1, alamNotificationBuilder.build());
        Log.d("RoadConditionService", "Notification sent.");
    }
}